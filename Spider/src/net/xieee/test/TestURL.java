...
public class TestURL {
	public static void main(String[] args)  {
		...
		String host = "img0.bdstatic.com";
		for (int i = 0; i < 1000; i++) {
			try {
				List<Picture> list = getPictureBuit("美女", i + 1, pageSize);
				for (Picture picture : list) {
					System.out.println(picture.toString());
					downloadImage.Download(picture, host, Constants.butiful_img_save_path, Constants.butiful_img_http_path);
				}
				Thread.sleep(5000);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static List<Picture> getPictureBuit(String keywords, int pageIndex,
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			if (jsonObj.get("objURL") != null) {
				...
				picture.setLocal_url(Constants.butiful_img_http_path+pictureName.substring(pictureName.lastIndexOf("/")+1,pictureName.length()));
				picture.setInter_url(pictureName);
				picture.setSpark_url(jsonObj.getString("fromURL"));
				picture.setKey_word(Constants.butiful_MM_keyword+"");
				picture.setInter_url_samll(jsonObj.getString("thumbURL"));
				list.add(picture);
				download(jsonObj.get("objURL").toString(),pictureName.substring(pictureName.lastIndexOf("/")+1,pictureName.length()), Constants.butiful_img_save_path);
			}
		}
		return list;
	}
